...
| 2026-09-12 | C-1…C-3 | 9a6f576, 8387256, e3707c5 | Device catalog (families/models/issues/offerings), catalog admin + price matrix, public /services pages, committed price-free seed (`seed_catalog`), ISR revalidation, server-only page-data key | client | — | n/a |
| 2026-09-12 | C-1…C-3 | 62cc75b | Stopped tracking `research/ifixit-crawl/` (competitor crawl was committed in a019d34; repo is public). Files remain in git history until the owner decides on a history rewrite | client | — | n/a |
| 2026-09-12 | C-1…C-3 | 22fdd1a | Token refresh: blacklisted refresh token → 401 (was 500) without clearing the cookie (concurrent-refresh race) | generic | applelab→template | PENDING (BP paused) |
| 2026-09-14 | C-1…C-3 deploy | b71253d | Production routing behind nginx: Django admin moved to `/django-admin/` (Next.js staff panel owns `/admin`); `/api/` added to `SECURE_REDIRECT_EXEMPT` so loopback server-side fetches aren't 301'd to https. settings.py/urls.py rows apply manually on the template side | generic | applelab→template | PENDING (BP paused) |
